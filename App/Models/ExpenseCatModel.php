...
class ExpenseCatModel extends \Core\Model
{
    ...
    { 
        ...
        if(empty($this->errors)) {
            ...
            $sql = 'INSERT INTO expenses_category_assigned_to_users (user_id, name, maximum)
                    VALUES (:user_id, :name, :maximum)';
            $db = static::getDB();
            $stmt = $db->prepare($sql);
            $stmt->bindValue(':user_id', $_SESSION['user_id'], PDO::PARAM_INT);
            $stmt->bindValue(':name', $this->name, PDO::PARAM_STR);
            $stmt->bindValue(':maximum', $this->maximum, PDO::PARAM_STR);
            return $stmt->execute();
    ...
    public static function getExpenseLimit($name)
    {
        $sql = 'SELECT * 
                FROM expenses_category_assigned_to_users 
                WHERE user_id = :id 
